...
export default class ColorSettings extends FormApplication {
  ...
  activateListeners(html) {
    ...
    $(html)
      ...
      .change((event) => {
        ...
        if (template) {
          ...
          const key = this.localActor
            ? getDispositionName(this.localToken, this.localActor)
            : event.currentTarget.attributes.getNamedItem("[data-id]").value;
    ...
    $(html)
      .find("button.default-button")
      .click((event) => {
        event.preventDefault();
        const type = event.currentTarget.attributes.getNamedItem("[data-id]").value;

        const defaultColor =
          type === "actor" ? getActorIntroductionColorsByData(this.localToken, this.localActor) : DefaultColors[type];
